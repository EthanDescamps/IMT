...
{	public:
	...
	T& operator[] (unsigned); 			// element access operator (non-const)
	const T& operator[] (unsigned) const; 	// element access operator (const)
	T& operator() (unsigned); 			// element access operator (non-const) - one-based
	const T& operator() (unsigned) const; 	// element access operator (const) - one-based
...
#include "vector.tpp" // detailed template definitions (moved from vector.cpp)
