...
// Note: the specialization Vector<double>::Vector(const Matrix&) is
// implemented in a single translation unit (matrix.cpp) to avoid
// multiple-definition linker errors when this header is included in
...
template<typename T> 		// element access operator (non-const)
T& Vector<T>::operator[](const unsigned _index)
{ return vData[_index]; }

template<typename T> 		// element access operator (const)
const T& Vector<T>::operator[](const unsigned _index) const
{ return vData[_index]; }
